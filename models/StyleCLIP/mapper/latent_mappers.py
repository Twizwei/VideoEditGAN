...
class Mapper_1024(Module):

    def __init__(self, opts):
        super(Mapper_1024, self).__init__()

        self.opts = opts
        layers = [PixelNorm()]

        layers.append(EqualLinear(1024, 512, lr_mul=0.01, activation='fused_lrelu'))
        for i in range(1, 4):
            layers.append(
                EqualLinear(
...
class LevelsMapper_1024(Module):

    def __init__(self, opts):
        super(LevelsMapper_1024, self).__init__()

        self.opts = opts

        if not opts.no_coarse_mapper:
            self.course_mapping = Mapper_1024(opts)
        if not opts.no_medium_mapper:
            self.medium_mapping = Mapper_1024(opts)
        if not opts.no_fine_mapper:
            self.fine_mapping = Mapper_1024(opts)
    ...
    def forward(self, x):
        ...
        if not self.opts.no_coarse_mapper:
            ...
            x_coarse = torch.zeros_like(x_coarse)[..., :512]
        if not self.opts.no_medium_mapper:
            x_medium = self.medium_mapping(x_medium)
        else:
            x_medium = torch.zeros_like(x_medium)[..., :512]
        if not self.opts.no_fine_mapper:
            x_fine = self.fine_mapping(x_fine)
        else:
            x_fine = torch.zeros_like(x_fine)[..., :512]
