...
def run_stit(test_opts, model_path, image_name, use_multi_id_G, itr, factor=0.06):
    # out_path_results = os.path.join(test_opts.exp_dir, test_opts.data_dir_name)
    # os.makedirs(out_path_results, exist_ok=True)
    # out_path_results = os.path.join(out_path_results, test_opts.image_name)
    # os.makedirs(out_path_results, exist_ok=True)
    ...
    new_G = ckpt['generator'].to(global_config.device).eval()
    latent = ckpt['pivots'][itr:itr+1]
    new_G = new_G.float()
    toogle_grad(new_G, False)
    
    latent_code = run_styleclip(net, new_G, opts, paths_config.pti_results_keyword, out_path_results, test_opts, factor=factor, latent=latent)
    return latent_code

def run_styleclip(net, G, opts, method, out_path_results, test_opts, factor=0.06, latent=None):
    net.set_G(G)
