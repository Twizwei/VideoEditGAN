...
def project(
    ...
    assert target.shape == (G.img_channels, G.img_resolution, G.img_resolution), 'Target shape is ' + str(target.shape) + 'but G shape is ' + str(G.img_channels) + ' ' + str(G.img_resolution)
    ...
    del G
    return w_opt.repeat([1, 18, 1])  # manually repeat 18 times to get to W+ space
